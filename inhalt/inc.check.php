    ...
    if ($datensaetze2 == 1) {
        ...
        if ($phase == 1) {
            if ((preg_match("/kommunikation_exch/i", $SCRIPT_NAME)) or (preg_match("/kommunikation_exch/i", $SCRIPT_FILENAME))) {
                header("Location: kommunikation_exch.php?fu=7&sprache=" . $_GET['sprache']);
                exit;
            } else {
                header("Location: runde_ende.php?fu=1&spiel=$spiel&sprache=" . $_GET['sprache']);
                exit;
            }
        }
        ...
        if ((preg_match("/kommunikation_exch/i", $SCRIPT_NAME)) or (preg_match("/kommunikation_exch/i", $SCRIPT_FILENAME))) {
            ...
            header("Location: ../index.php?sprache=" . $_GET['sprache']);
            exit;
        }
    }
} else {
