...
<form name="formular" id="formular" method="post" action="start.php?action=create">
...
<?
    ...
    $sql = "SELECT spiel_name, spieler_admin, user_1, user_2, user_3, user_4, user_5, user_6, user_7, user_8, user_9, user_10 FROM skrupel_waiting_games WHERE id = {$_POST["game_id"]}";
    ...
    if (!in_array($_SESSION["user_id"], array($data["user_1"],
        ...
        for ($i = 1; $i <= 10; $i++) {
            if ($spiel["user_" . $i] == -1) {
                $sql = "UPDATE skrupel_waiting_games SET user_$i = 0 WHERE id = {$_POST["game_id"]} AND user_$i = -1";
                mysql_query($sql);
            }
        }
        $mailsto = "";
        ...
        $sql = "DELETE FROM skrupel_waiting_games WHERE id = {$_POST["game_id"]}";
        mysql_query($sql);
        
        /*$ftploginname = $_COOKIE["ftploginname"] = $_COOKIE["ftploginname"] = $admin_login;
        $ftploginpass = $_COOKIE["ftploginpass"] = $_COOKIE["ftploginpass"] = $admin_pass;
        $_GET["fu"] = 9; //0.969
        $_GET["fu"] = 9; //0.97+
        require $conf_root_path . "admin/spiel_alpha.php";*/
...
?>
<center><br><br>Das Spiel wurde erfolgreich erstellt.<br><br>
<a href="active.php">Weiter</a></center>
<?php
        //////////////////////////////////////////////////////
