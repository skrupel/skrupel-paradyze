...
if(count($sprachen) == 0) {
    include(PRDYZ_DIR_LANG.$language.'/lang.inc.host.php');
} else {
    foreach($sprachen as $sprache) {
        include(PRDYZ_DIR_LANG.$sprache.'/lang.inc.host.php');
    }
}
...
$handle = opendir(PRDYZ_DIR_DATA);

if ($handle) {
    while ($rasse = readdir($handle)) {
        if (is_dir(PRDYZ_DIR_DATA.$rasse) && is_file(PRDYZ_DIR_DATA.$rasse.'/daten.txt')) {
            $rassendaten = rasse_laden(PRDYZ_DIR_DATA.$rasse.'/daten.txt');
...
if ($planetenanzahl>0) {
    include(PRDYZ_DIR_INCLUDE.'inc.host_bodenkampf.php');
}
...
if ($schiffanzahl>=1) {
    for  ($i=0; $i<$schiffanzahl;$i++) {
        ...
        $verbrauchpromonat = getVerbrauchProMonat($antrieb);
			...
			$n_gescannt=0;
                ...
                if($anzahl_temp2 > 0){
                    for($j=0;$j< $anzahl_temp2; $j++){
                        $array_temp2 = mysql_fetch_array($zeiger_temp2,$j);
...
///////////////////////////////////////////////////////////////////////////////////////////////SPIONAGE ANFANG
if($module[0]) {
    include(PRDYZ_DIR_INCLUDE.'inc.host_spionage.php');
}
///////////////////////////////////////////////////////////////////////////////////////////////SPIONAGE ENDE
...
include(PRDYZ_DIR_INCLUDE.'inc.host_orbitalkampf.php');

///////////////////////////////////////////////////////////////////////////////////////////////SCHIFFSKAMPF PLANET ENDE

///////////////////////////////////////////////////////////////////////////////////////////////SCHIFFSKAMPF ANFANG

include(PRDYZ_DIR_INCLUDE.'inc.host_raumkampf.php');
                ...
                if ($planetenanzahl==1) {
                    ...
                    if(($osys_1!=19) and ($osys_2!=19) and ($osys_3!=19) and ($osys_4!=19) and ($osys_5!=19) and ($osys_6!=19)){
                        if ($beziehung[$besitzer][$p_besitzer]['status']!=5) {
                            ...
                            if (($spieler_1>=1) and ($p_besitzer<>1)) { neuigkeit(4,"../bilder/news/star_explode.jpg",1,$lang['host'][$spielersprache[1]]['destabilisator'][0]); }
                            if (($spieler_2>=1) and ($p_besitzer<>2)) { neuigkeit(4,"../bilder/news/star_explode.jpg",2,$lang['host'][$spielersprache[2]]['destabilisator'][0]); }
                            if (($spieler_3>=1) and ($p_besitzer<>3)) { neuigkeit(4,"../bilder/news/star_explode.jpg",3,$lang['host'][$spielersprache[3]]['destabilisator'][0]); }
                            if (($spieler_4>=1) and ($p_besitzer<>4)) { neuigkeit(4,"../bilder/news/star_explode.jpg",4,$lang['host'][$spielersprache[4]]['destabilisator'][0]); }
                            if (($spieler_5>=1) and ($p_besitzer<>5)) { neuigkeit(4,"../bilder/news/star_explode.jpg",5,$lang['host'][$spielersprache[5]]['destabilisator'][0]); }
                            if (($spieler_6>=1) and ($p_besitzer<>6)) { neuigkeit(4,"../bilder/news/star_explode.jpg",6,$lang['host'][$spielersprache[6]]['destabilisator'][0]); }
                            if (($spieler_7>=1) and ($p_besitzer<>7)) { neuigkeit(4,"../bilder/news/star_explode.jpg",7,$lang['host'][$spielersprache[7]]['destabilisator'][0]); }
                            if (($spieler_8>=1) and ($p_besitzer<>8)) { neuigkeit(4,"../bilder/news/star_explode.jpg",8,$lang['host'][$spielersprache[8]]['destabilisator'][0]); }
                            if (($spieler_9>=1) and ($p_besitzer<>9)) { neuigkeit(4,"../bilder/news/star_explode.jpg",9,$lang['host'][$spielersprache[9]]['destabilisator'][0]); }
                            if (($spieler_10>=1) and ($p_besitzer<>10)) { neuigkeit(4,"../bilder/news/star_explode.jpg",10,$lang['host'][$spielersprache[10]]['destabilisator'][0]); }
...
if ($planetenanzahl>=1) {
    ...
    for ($i=0; $i<$planetenanzahl;$i++) {
        ...
        if ($native_fert_klau==5) {
            $mineral_klau=round($vorrat_neu/100*mt_rand(30,80));
        }
        ...
        if ($auto_fabriken==1) {
            ...
            if ($max_fabriken>$max_col) { $max_fabriken=$max_col;$max_bau=max(0,$max_col-$fabriken);}
        ...
        if ($auto_minen==1) {
            ...
            if ($max_minen>$max_col) { $max_minen=$max_col;$max_bau=max(0,$max_col-$minen);}
        ...
        if ($auto_abwehr==1) {
            ...
            if ($max_abwehr>$max_col) { $max_abwehr=$max_col;$max_bau=max(0,$max_col-$abwehr);}
...
include(PRDYZ_DIR_INCLUDE.'inc.host_nebel.php');
...
if ($endejetzt==1) {
    include(PRDYZ_DIR_INCLUDE.'inc.host_spielende.php');
}
...
for ($k=1; $k<=10; $k++) {
    if ($spieler_id_c[$k]>0 and $spieler_raus_c[$k]==0) {
        ...
        $nachrichtmessenger=str_replace('{1}',$spiel_name,$lang['host'][$spielersprache[$k]]['letztermonat'][24]);
        $emailtopic=str_replace(array('{SERVER}', '{1}'),array($conf_game_title, $spiel_name),$lang['host'][$spielersprache[$k]]['letztermonat'][20]);
		...
		// Nachricht ueber ICQ, funktioniert nicht mehr
        /*
        if (substr($optionen,1,1)=='1') {
            $header="From $absenderemail\nReply-To:$absenderemail\n";
            @mail($emailicq,$emailtopic,"$nachrichtmessenger",$header);
        }
        */
