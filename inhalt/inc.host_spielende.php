...
if ($ziel_id==6) {
	...
	$zeiger_temp = mysql_query("SELECT name FROM skrupel_spiele WHERE id=$spiel");
	$array = mysql_fetch_assoc($zeiger_temp);
	$spielname = $array['name'];
    if ($siegeranzahl>1) {
        $gewinner="";
        for ($n=0;$n<$siegeranzahl;$n++) {
            $gewinner=$gewinner.$sieger[$n];
            if ($n<$siegeranzahl-2) { $gewinner=$gewinner.", "; }
            if ($n==$siegeranzahl-2) { $gewinner=$gewinner." und "; }
			$nachricht_messenger = "Das Spiel '$spielname' ist beendet. Die Gewinner sind $gewinner.";
        }
    } else {
        $gewinner=$sieger[0];
		$nachricht_messenger = "Das Spiel '$spielname' ist beendet. Der Gewinner ist $username.";
    }

	// Nachricht auf Twitter "zwitschern"
	include_once('classes/class.twitter.php');
	$twt = new Twitter('Paradyze_Game', 'De$t!n4tIoN#uNkN0wN');
	$twt->updateStatus(stripslashes($nachricht_messenger));
