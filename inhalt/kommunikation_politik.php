...
<?php
                                    ...
                                    foreach($array_spieler as $i => $spieler) {
                                        $zahl++;
                                        if (($module[4]==0) or ($begegnung[1]==1)) {
                                            if (($array_spieler[$i]>=1) and ($i<>$spieler) and ($spieler!=$spieler_id)) {
                                                $zeiger = @mysql_query("SELECT * FROM skrupel_user where id=$array_spieler[$i]");
                                                $array = @mysql_fetch_array($zeiger);
?>
                                                    <option value="<?php echo $zahl; ?>" style="background-color:<?php echo $spielerfarbe[$zahl]; ?>;"><?php echo $array["nick"]; ?></option>
<?php
                                            }
                                        }
                                    }
